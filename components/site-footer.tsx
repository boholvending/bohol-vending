...
export function SiteFooter({ locale = "en" }: { locale?: "en" | "zh" }) {
  ...
  return <footer className={s.footer}>
    <div className={`${s.panel} ${s.about}`}><Link className={s.logo} href={zh?"/zh":"/"} aria-label="BOHOL home"><Image src="/images/brand/bohol-logo.webp" alt="BOHOL" width={500} height={500} sizes="108px"/></Link><p>{zh?"BOHOL 专注智能自动售货设备的研发与制造，为品牌方、运营商和经销商提供专业产品及 OEM/ODM 定制服务。":"BOHOL designs and manufactures intelligent vending machines, providing specialized products and OEM/ODM solutions for brands, operators and distributors worldwide."}</p></div>
    ...
    <div className={s.panel}><h2>{zh?"网站导航":"SUPPORT"}</h2><nav><span><Link href={zh?"/zh":"/"}>{zh?"首页":"Home"}</Link></span>{!zh && <span><Link href="/vending">Vending</Link></span>}<span><Link href={path("/about")}>{zh?"关于 BOHOL":"About Us"}</Link></span><span><Link href={path("/solutions")}>{zh?"解决方案":"Solutions"}</Link></span><span><Link href={path("/oem-odm")}>OEM &amp; ODM</Link></span><span><Link href={path("/projects")}>{zh?"成功案例":"Projects & Cases"}</Link></span><span><Link href={path("/insights")}>{zh?"新闻资讯":"Insights & FAQ"}</Link></span></nav></div>
