...
export function MainNavigation({locale="en"}:{locale?:"en"|"zh"}){
  ...
  return <><details className={s.mobile}><summary aria-label={zh?"打开导航":"Open navigation"}>{zh?"菜单":"Menu"} <ChevronDown size={16}/></summary><nav aria-label={zh?"手机导航":"Mobile navigation"}>{[["",zh?"首页":"Home"],["vending-machines",zh?"自动售货机":"Vending Machines"],["solutions",zh?"解决方案":"Solutions"],["oem-odm","OEM & ODM"],["projects",zh?"项目案例":"Projects & Cases"],["contact",zh?"联系方式":"Contact"],["insights",zh?"行业洞察":"Insights"],["about",zh?"关于 BOHOL":"About BOHOL"]].map(([path,label])=><Link key={path} href={`${root}/${path}`}>{label}</Link>)}</nav></details><nav className={s.nav} aria-label={zh?"主导航":"Primary navigation"}>
